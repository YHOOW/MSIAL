...
if (nargin < 5)
    ...
    %alpha函数，设置在当前坐标轴上设置对象的透明度属性。使用格式：
...
%size(Y,2)返回的是矩阵Y的列数，size(Y,1)返回的是矩阵Y的行数；
N = size(Y,2);
...
if (~affine)
    % initialization
    % inv函数用来求逆矩阵，Y’是Y的转置矩阵；eye()函数返回的是单位矩阵，eye（N）返回N*N大小的单位矩阵；
    A = inv(mu1*(Y'*Y)+mu2*eye(N));
    % zeros是一个数组，zeros(N)：生成一个N*N的零方阵，zeros（M,N）或zeros([M,N])：生成一个M*N的零矩阵。
    C1 = zeros(N,N);
    Lambda2 = zeros(N,N);
